...
def gerar_excel_analise(df):
    ...
    # Análise por faixas (BASEADA EM VALOR)
    valor_ate_44 = df[df['Cobertura Atual'] <= 44]['Saldo Pedido'].sum()
    valor_entre_45_70 = df[(df['Cobertura Atual'] >= 45) & (df['Cobertura Atual'] <= 70)]['Saldo Pedido'].sum()
    valor_acima_71 = df[df['Cobertura Atual'] >= 71]['Saldo Pedido'].sum()
    
    perc_valor_ate_44 = (valor_ate_44 / valor_total) * 100 if valor_total > 0 else 0
    perc_valor_45_70 = (valor_entre_45_70 / valor_total) * 100 if valor_total > 0 else 0
    perc_valor_acima_71 = (valor_acima_71 / valor_total) * 100 if valor_total > 0 else 0
    ...
    for row, (_, item) in enumerate(df_ordenado.iterrows(), 2):
        ...
        dados = [
            ...
            item['Saldo Pedido'],  # Formatado como número
            f"{cobertura:.1f}",
        ...
        for col, valor in enumerate(dados, 1):
            ...
            if col == 8:  # Coluna Saldo Pedido
                cell.number_format = '#,##0.00'
            elif col == 9:  # Coluna Cobertura Atual
                cell.number_format = '0.0'
            elif col == 7:  # Coluna Quantidade
...
def gerar_resumo_analise(df):
    ...
    for fornecedor in df['Fornecedor'].unique():
        ...
        valor_acima_71_forn = dados_forn[dados_forn['Cobertura Atual'] >= 71]['Saldo Pedido'].sum()
        valor_total_forn = dados_forn['Saldo Pedido'].sum()
        perc_acima_71_forn = (valor_acima_71_forn / valor_total_forn) * 100 if valor_total_forn > 0 else 0
    ...
    resumo = {
        ...
        'distribuicao_faixas': {
            'ate_44_dias': {'valor': valor_ate_44, 'percentual': perc_valor_ate_44},
            'entre_45_70_dias': {'valor': valor_entre_45_70, 'percentual': perc_valor_45_70},
            'acima_71_dias': {'valor': valor_acima_71, 'percentual': perc_valor_acima_71}
        },
        'recomendacoes': {
...
def criar_aba_faixas_por_filial(wb, df):
    ...
    headers = [
        ...
        'Valor Até 44d', '% Valor 44d', 'Valor 45-70d', '% Valor 45-70d',
        'Valor >71d', '% Valor >71d'
    ]
...
def criar_aba_faixas_fornecedor_filial(wb, df):
    ...
    for fornecedor in df['Fornecedor'].unique():
        ...
        for filial in dados_forn['Filial'].unique():
            ...
            # Distribuição por faixas (BASEADA EM VALOR)
            valor_ate_44 = dados_forn_filial[dados_forn_filial['Cobertura Atual'] <= 44]['Saldo Pedido'].sum()
            valor_entre_45_70 = dados_forn_filial[(dados_forn_filial['Cobertura Atual'] >= 45) & (dados_forn_filial['Cobertura Atual'] <= 70)]['Saldo Pedido'].sum()
            valor_acima_71 = dados_forn_filial[dados_forn_filial['Cobertura Atual'] >= 71]['Saldo Pedido'].sum()
            ...
            combinacoes_analise.append({
                ...
                'valor_total': valor_total,
                'cobertura_media': cobertura_media,
                'valor_ate_44': valor_ate_44,
                'perc_valor_ate_44': perc_valor_ate_44,
                'valor_entre_45_70': valor_entre_45_70,
                'perc_valor_45_70': perc_valor_45_70,
                'valor_acima_71': valor_acima_71,
                'perc_valor_acima_71': perc_valor_acima_71
            })
    
    # Ordenar por % acima de 71 dias (mais críticos primeiro)
    combinacoes_analise.sort(key=lambda x: x['perc_valor_acima_71'], reverse=True)
    ...
    for row, comb_data in enumerate(combinacoes_analise, 4):
        dados = [
            ...
            comb_data['filial'],
            comb_data['total_itens'],
            comb_data['valor_total'],  # Valor como número
            comb_data['cobertura_media'],  # Número
            comb_data['valor_ate_44'],  # Valor como número
            comb_data['perc_valor_ate_44'],  # Percentual como número
            comb_data['valor_entre_45_70'],  # Valor como número
            comb_data['perc_valor_45_70'],  # Percentual como número
            comb_data['valor_acima_71'],  # Valor como número
            comb_data['perc_valor_acima_71']  # Percentual como número
        ]
        ...
        for col, valor in enumerate(dados, 1):
            ...
            # Formatação de números
            if col in [4, 6, 8, 10]:  # Colunas de valor
                cell.number_format = '#,##0.00'
            elif col in [5]:  # Coluna de cobertura média
                cell.number_format = '0.0'
            elif col in [7, 9, 11]:  # Colunas de percentual
                cell.number_format = '0.0"%"'
            
            # Colorir baseado na criticidade
            if comb_data['perc_valor_acima_71'] > 50:
                cell.fill = PatternFill(start_color="FFD6D6", end_color="FFD6D6", fill_type="solid")
            elif comb_data['perc_valor_acima_71'] > 25:
                cell.fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
            else:
                cell.fill = PatternFill(start_color="E6F3E6", end_color="E6F3E6", fill_type="solid")
